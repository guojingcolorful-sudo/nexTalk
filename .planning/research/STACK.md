...
**Domain:** 实时 AI 同传面试辅助系统 (real-time Chinese→English voice translation desktop app with voice cloning + AI interview copilot)
**Researched:** 2026-08-26
**Confidence:** MEDIUM-HIGH (vendor landscape verified against multiple 2026 sources; per-vendor quality claims for Chinese→English voice cloning must be validated by a phase-1 listening test)

## Executive Verdict: 2026 Vendor Re-Evaluation

The requirement doc's 2024-25 picks (Deepgram / GPT-4o-mini / GPT-4o+Claude 3.5 / ElevenLabs / Tavily) are all **superseded or beaten on price/quality** in 2026. The biggest 2026 development is a new category: **unified speech-to-speech translation models** (Gemini 3.5 Live Translate, gpt-realtime-translate, Azure Live Interpreter + Personal Voice) — but none of them satisfies this product's core requirement (clone the *user's* voice, ≤2s e2e, partial text streams for the teleprompter). **The cascaded pipeline stays the recommended architecture**, with each stage swapped to a 2026-class provider:
...
Keep the cascade: it is the only architecture satisfying clone-quality, ≤2s budget, partial-text streams for subtitles/AI-editing, and vendor independence — all at once. Re-evaluate Azure Personal Voice mid-milestone if approval is obtainable.

**Latency budget check (cascade, worst case):** STT partial (~300-600ms) → LLM TTFT (~150ms) → TTS TTFA (MiniMax ~250ms / Cartesia ~90ms) ≈ **1.2-1.6s** e2e for first audio → within the 2s budget. The unified models run 2.9s+ — they are *slower*, not faster.
...
pnpm create tauri-app@latest nextalk --template react-ts

# Frontend deps
pnpm add react react-dom
pnpm add tailwindcss@3.4 @tailwindcss/vite  # v3.4, NOT v4 — Safari 15.6 floor
pnpm add @tauri-apps/api @tauri-apps/plugin-global-shortcut
pnpm add -D typescript vite@7 vitest @playwright/test

# Rust deps (cargo add, inside src-tauri)
cargo add cpal webrtc-audio-processing@~2.0 rubato hound
cargo add tokio-tungstenite axum tokio --features tokio/full
cargo add reqwest --features stream,json
cargo add silero-vad-rs qrcode
```
...
**If user's region is mainland China with unstable international latency (~+100-200ms RTT):**
...
**If Azure Personal Voice limited-access approval is granted:**
...
**If the Gemini Live text-modality interim transcripts are unusable for cascading:**
...
**If MiniMax clone quality fails the phase-1 listening test:**
...
**If the copilot's 1.5s strategy budget is exceeded by Gemini 3.5 Flash:**
...
**Unified S2S translation models**
...
**Open questions flagged for phase-1 validation:**
1. Actual Chinese WER of `gemini-3.1-flash-live` text modality vs Deepgram Nova-3 on real interview audio (μ-Bench shows big variance).
2. Chinese→English clone listening test: MiniMax 2.6 Turbo vs Cartesia Sonic 3.5 vs Fish S2.1 Pro vs ElevenLabs Flash (blinded, ~20 short answers, 3 raters).
3. Network RTT from the user's region to each provider (Deepgram/ElevenLabs are US-centric; MiniMax/Fish have global edge).
4. Gemini Live API text modality exact model string + interim-transcript cadence (docs change fast).
5. End-to-end latency measurement on a real Monterey 12.7 box (Intel + M1).

---
*Stack research for: NexTalk 极言 — real-time AI voice translation / interview copilot*
*Researched: 2026-08-26*
