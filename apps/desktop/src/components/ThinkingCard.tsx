 ...
 * newest question awaits its strategy, so the AI pane reads as live thinking
...
/**
 * ThinkingCard — the AI 思考中 state for the newest question: a pulsing bolt,
...
export default function ThinkingCard({ nodeRef }: { nodeRef?: Ref<HTMLDivElement> }) {
  ...
  return (
    <div
      ...
      role="status"
      aria-label="AI 思考中"
      className="mt-3 flex items-start gap-2 self-start rounded-xl border-4 border-black bg-white px-3 py-2 shadow-cartoon-yellow"
    >
      <FontAwesomeIcon
        icon={faBolt}
        aria-hidden="true"
